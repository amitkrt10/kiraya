...
--
-- Purpose:
-- Applies an existing tenant credit to a bill.
--
-- Example:
--
-- Tenant credit       ₹5,000
...
-- Remaining bill       ₹25,000
--
-- The original payment and original bill are never modified.
-- ============================================================
...
begin
    ...
    if v_bill.status not in ('DRAFT', 'FINALIZED') then
        raise exception
            using
                errcode = '23514',
    ...
    v_credit :=
        kiraya.get_tenant_credit(v_bill.tenant_id);

    v_bill_balance :=
        greatest(
            0,
            v_bill.total_amount
            - kiraya.get_bill_paid_amount(v_bill.id)
        );
...
comment on function kiraya.apply_tenant_credit_to_bill(uuid, numeric, uuid) is
    'Applies available tenant credit against a bill without modifying the original payment.';
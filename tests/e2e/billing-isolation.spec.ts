 ...
 * different organizations, plus a billing run and bill already existing
 * under organization A. Populate these to run it:
 *
 ...
 *   E2E_ORG_A_RUN_ID                      — a billing run id belonging to org A
 *   E2E_ORG_A_BILL_ID                     — a bill id belonging to org A
...
const orgARunId = process.env.E2E_ORG_A_RUN_ID;
const orgABillId = process.env.E2E_ORG_A_BILL_ID;
const orgBEmail = process.env.E2E_ORG_B_EMAIL;
const orgBPassword = process.env.E2E_ORG_B_PASSWORD;

const hasCredentials = Boolean(orgAEmail && orgAPassword && orgARunId && orgABillId && orgBEmail && orgBPassword);
...
test.describe("cross-organization billing isolation", () => {
  test.skip(
    !hasCredentials,
    "Needs E2E_ORG_A_EMAIL/PASSWORD/RUN_ID/BILL_ID and E2E_ORG_B_EMAIL/PASSWORD for two real, different-organization users — not available in this environment.",
  );
